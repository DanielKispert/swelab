...
public class Lernspielmanager extends Brettspielmanager implements IntLernspielmanager {
	
	private final IntLernspielSpieler[] spieler;
	
	private final IntKategorie[] kategorien;

	private final IntFeld[] felder;
	
	private int gewürfelt;
	
	private int anzahlWürfe;
	
	private IntLernspielSpieler amZug;
	...
	private IntKategorie ausgewählteKategorieFürFrage;
	
	private IntLernspielSpieler gefragterSpieler;
	
	private IntFrage frage;
	
	private IntLernspielSpieler hatGewonnen;

	public Lernspielmanager(Spielerinfos[] infos, IntKategorie[] kategorien) {
		this.spieler = new LernspielSpieler[infos.length];
		for (int i = 0; i < infos.length; i++) {
			this.spieler[i] = new LernspielSpieler(infos[i].getName(), infos[i].getFarbe(), kategorien);
		}
	...
	private int getStartfeldVonSpieler(IntLernspielSpieler spieler) {
		int startfeldId = -1;
	 ...
	 */
	private void stelleFrage(IntLernspielSpieler gefragterSpieler) {
		//TODO
	...
	public void zieheAufFeld(int id) {
		IntLernspielSpieler besetzenderSpieler = this.felder[id].getBesetztVon();
	...
	public String getWissensstandVon(IntLernspielSpieler spieler) {
		for (IntLernspielSpieler player: this.spieler) {
